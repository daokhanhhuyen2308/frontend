...
//     }
// }
// Cập nhật hàm addDataToHTML
const addDataToHTML = function (productFilter) {
    // Xác định vị trí bắt đầu và kết thúc của sản phẩm trên trang hiện tại
    let begin = limit * (thisPage - 1);
    let end = limit * thisPage;

    // Lấy ra các sản phẩm trên trang hiện tại bằng cách cắt phần tử từ mảng listProducts
    const pageProducts = productFilter.slice(begin, end);

    count.innerText = pageProducts.length;

    // Xóa dữ liệu hiện có trong listProductHTML
    listProductHTML.innerHTML = '';
    ...
    if (pageProducts.length > 0) {
        ...
        listPage(pageProducts);
    }

}

function listPage(pageProducts) {
    let count = Math.ceil(pageProducts.length / limit);
...
//     // Tính toán số lượng trang dựa trên số lượng sản phẩm và số lượng sản phẩm trên mỗi trang
//     const totalPages = Math.ceil(productFilter.length / limit);

//     // Tạo nút hoặc liên kết cho mỗi trang
